<template>
    <!-- PaIcon -->
    <PaIcon class="ba-icon-dark" v-if="field.render == 'icon'" :name="fieldValue ? fieldValue : field.default ?? ''" />
    ...
    <div v-if="field.render == 'url' && fieldValue">
        <el-input :model-value="fieldValue" placeholder="Link address">
            <template #append>
                ...
                >
                    <PaIcon :color="'#606266'" name="el-icon-Position" />
                </el-button>
            </template>
        </el-input>
    </div>
...
import { timeFormat, openUrl } from '@/components/PaTable'
// import { arrayFullUrl } from '@/utils/common'
...
const onChangeField = (value: any) => {
    paTable.onTableAction('field-change', { value: value, ...props })
}
...
const onButtonClick = (btn: OptButton) => {
    ...
    paTable.onTableAction(btn.name, props)
}
